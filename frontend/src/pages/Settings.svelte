<script lang="ts">
  ...
  // Opens the global upgrade modal (lives in App.svelte so the topbar badge
  // can trigger it from any page without navigating here first).
  ...
  {#if upd}
    <div class="upd">
      ...
      {:else if upd.hasUpdate}
        ...
        {#if info?.elevated}
          <button class="link" on:click={onUpgrade}>一键升级</button>
